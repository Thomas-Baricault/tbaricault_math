...
namespace tbaricault::uniconvert
{
    ...
    template<typename T>
    tbaricault::math::Rect<T> Converter<std::string, tbaricault::math::Rect<T>>::operator()(const std::string& value) const
    {
        std::vector<std::string> args = tbaricault::str::split(value, " ", false);
        if (args.size() == 4)
        {
            return tbaricault::math::Rect<T>(
                convert<std::string, T>(args.at(0)),
                convert<std::string, T>(args.at(1)),
                convert<std::string, T>(args.at(2)),
                convert<std::string, T>(args.at(3))
            );
        }
    ...
    template<typename T>
    std::string Converter<tbaricault::math::Rect<T>, std::string>::operator()(const tbaricault::math::Rect<T>& value) const
    {
        return (
            convert<T, std::string>(value.x) + ' ' +
            convert<T, std::string>(value.y) + ' ' +
            convert<T, std::string>(value.w) + ' ' +
            convert<T, std::string>(value.h)
        );
    }
